...
*/
	...
	Update(SourceIni, sections=true, keys=true, values=false, top_comments=false, section_comments=true, key_comments=true, repeatedRecursions=0) {  ; TODO: add docstring
		...
		if (top_comments) {
			for commentIndex, commentContent in StrSplit(SourceIni.GetTopComments(), "`n")  ; Add new top comment
				if (!InStr(this.GetTopComments(), commentContent))
					this.AddTopComment(commentContent)
		}
		for sectionName, sectionKeys in SourceIni {  ; Add new section
		...
		removeSectionsList := []
		for sectionName, sectionKeys in this {  ; Remove old section, remember Section, remove later to not mess up the object index.-
			...
			for keyName, keyVal in sectionKeys {  ; Remove old key
	...
	Compare(SourceIni, sections=true, keys=true, values=false, comments=false) {  ; TODO: add docstring
		if (!IsObject(SourceIni))
			SourceIni := class_EasyIni(SourceIni)
	...
	*/
	...
	Save(sSaveAs="", bWarnIfExist=false) {
		...
		for k, v in this.EasyIni_ReservedFor_TopComments  ; --- MODIFICATION START (dein0s) ---
		...
		if (!IsStringEmpty(TTempString:=Trim(TTempString,"`n"))) {
			...
			FileDelete, %sFile%
			FileAppend, % RegExReplace(TTempString,"[`r`n]{2,}","`n`n") "`n", %sFile%, UTF-8
		}
		return true
	}
